...
.unify_metadata <- function(list) {
  dplyr::bind_rows(list)
}

.rename_columns <- function(cols) {
...
.donor_matadata_modify <- function(tbl) {
  ...
  tbl <- tbl |>
    mutate(data_value = ifelse(.data$data_type == "Numeric",.data$data_value, ""),
      ...
      pivot_wider(
        ...
        values_fn = list("preferred_term" = function(x) paste(unique(x), 
                                                              collapse = "/"))) |>
      group_by(.data$hubmap_id) |>
      select(any_of(c(.default_columns("Donor", "character"), "Body mass index"))) |>
