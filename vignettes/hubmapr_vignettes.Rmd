...
entry of any entity category given **UUID**.
Use `select()` and `unnest_*()` functions to expand list-columns. It will be
convenient to view tables with multiple columns but one row using `glimpse()`.
...
There is a column named `"donor.hubmap_id"` in the retrieved tibble from
`datasets()` and `samples()`. This column can help to filter the retrieved
tibble from `donors()` to obtain corresponding donor UUID. Then, the donor
metadata can be obtained using `donor_metadata()`.

```{r 'donor metadata'}
# dataset uuid
donor_hubmap_id <- datasets |>
    filter(dataset_type == "Auto-fluorescence",
	    organ == "Kidney (Left)") |>
    head(1) |>
...
donor_uuid <- donors |>
    filter(hubmap_id == donor_hubmap_id) |>
    pull(uuid)
...
package, `sample_derived()` and `donor_derived()` functions are
...
`collection_contact()` and `collection_contributors()` for
**Collection** entity, or `publication_authors()` for **Publication**
entity.
...
files are available on HuBMAP Globus with corresponding URL.
...
Use `bulk_data_transfer()` to access data files. If the Globus URL is
