...
exploration and file retrieval via R.
...
information in the format of `"character"` or `"tibble"`. It will be convenient
to view tables with multiple columns but one row using `glimpse()`.

```{r 'cols'}
# as = "tibble" (default)
datasets_col_tbl <- datasets_default_columns(as = "tibble")
datasets_col_tbl |> glimpse()
...
Use `select()` and `unnest_*()` functions to expand list-columns.
...
Use `files_globus_url()` to access data files. If the Globus URL is 
